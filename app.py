...
.metric-sub { font-size: 11px; color: #7aaee8; margin-top: 3px; }
...
@st.cache_data
def load_data():
    df_raw = pd.read_excel("2015-2025git.xlsx", dtype=str, engine="openpyxl")

    # Avtomatično poišči pravi LETO posla stolpec (leta 2010-2030, min < 2019)
    leto_col_found = None
    for col in df_raw.columns:
        vals = pd.to_numeric(df_raw[col], errors="coerce").dropna()
        if len(vals) > 50:
            all_in_range = vals.between(2010, 2030).all()
            has_early = vals.min() < 2019
            if all_in_range and has_early:
                leto_col_found = col
                break

    df = df_raw.copy()

    # Preimenuj najdeni stolpec v LETO če še ni
    if leto_col_found and leto_col_found != "LETO":
        if "LETO" in df.columns:
            df = df.rename(columns={"LETO": "LETO_ORIG"})
        df = df.rename(columns={leto_col_found: "LETO"})
...
if "LETO" in df.columns:
    # Vse kombinacije kraj+leto z vsaj 1 poslom
    combos = (
        df[df["CENA_M2_UPR"] > 0]
        .groupby(["OBCINA", "LETO"])
        .size()
    ...
    ulica_z  = posel.get("ULICA", "")
    hisna_z  = posel.get("HISNA_STEVILKA", "")
    naslov   = f"{ulica_z} {hisna_z}".strip() or "neznan naslov"
    cena_z   = posel["CENA"]
