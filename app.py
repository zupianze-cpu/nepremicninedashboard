...
st.markdown("<div class='section-title'>Kakšna je realna cena nepremičnine?</div>",
            unsafe_allow_html=True)
st.markdown("Vnesi parametre — dobil boš oceno tržne vrednosti na podlagi primerljivih poslov.")

MIN_V = 5  # minimalni vzorec

rc1, rc2, rc3 = st.columns(3)
rc_obcina  = rc1.selectbox("Občina", [""] + sorted(df["OBCINA"].unique()), key="rc2_obcina")
rc_naselje = rc2.selectbox("Naselje", [""] + sorted(
    df[df["OBCINA"] == rc_obcina]["NASELJE"].unique() if rc_obcina else df["NASELJE"].unique()
), key="rc2_naselje")
rc_ulica   = rc3.selectbox("Ulica", [""] + sorted(
...
rc_hisna       = rd1.text_input("Hišna številka", "", key="rc2_hisna")
rc_upr         = rd2.number_input("Uporabna površina (m²)", 10, 1000, 60, 5, key="rc2_upr")
rc_skupna      = rd3.number_input("Skupna površina (m²)", 10, 1000, 65, 5, key="rc2_skupna")
rc_leto_izgr2  = rd4.number_input("Leto izgradnje", 1800, 2025, 1990, 1, key="rc2_leto")
rc_parcela2    = rd5.number_input("Površina atrija/parkinga (m²)", 0, 5000, 0, 5, key="rc2_parcela")

if st.button("🔍 Oceni vrednost", key="rc2_btn"):
    # Baza = vsi podatki, ni odvisna od sidebar filtrov
    baza2 = df.copy()

    # Lokacijski fallbacki
    lokacije2 = []
    if rc_ulica:
        lokacije2.append(("ulica", baza2[baza2["ULICA"] == rc_ulica]))
    if rc_naselje:
        lokacije2.append(("naselje", baza2[baza2["NASELJE"] == rc_naselje]))
    if rc_obcina:
        lokacije2.append(("občina", baza2[baza2["OBCINA"] == rc_obcina]))
    lokacije2.append(("Slovenija", baza2))

    def filtriraj2(vzorec, upr, leto):
        # Površina ±10%
        v = vzorec[
            vzorec["POVRSINA_ZA_IZRACUN"].between(upr * 0.90, upr * 1.10) &
            vzorec["LETO_IZGRADNJE_DELA_STAVBE"].between(leto - 10, leto + 10)
        ]
        if len(v) >= MIN_V:
            return v, "±10% površine, ±10 let"
        # Površina ±25% fallback
        v2 = vzorec[
            vzorec["POVRSINA_ZA_IZRACUN"].between(upr * 0.75, upr * 1.25) &
            vzorec["LETO_IZGRADNJE_DELA_STAVBE"].between(leto - 15, leto + 15)
        ]
        return v2, "±25% površine, ±15 let"

    def ocisti(vzorec, col):
        """Izreži top 10% in bottom 10%."""
        p10 = vzorec[col].quantile(0.10)
        p90 = vzorec[col].quantile(0.90)
        return vzorec[vzorec[col].between(p10, p90)]

    def izracunaj(vzorec, upr, skupna, parcela):
        """
        Izračun:
        - Posli brez parcele: cena = avg_m2_upr * upr + avg_m2_skupna * (skupna - upr)
        - Posli s parcelo:    cena = avg_m2_skupna_plus_parc * (skupna + parcela)
        """
        if parcela > 0:
            # Samo posli s PARCELA > 0
            v = vzorec[vzorec["PARCELA"].fillna(0) > 0].copy()
        else:
            # Samo posli brez parcele
            v = vzorec[vzorec["PARCELA"].fillna(0) == 0].copy()

        if len(v) < MIN_V:
            return None, None, None, None, len(v), "premalo vzorcev"

        v = ocisti(v, "CENA_M2_UPR")
        if len(v) < 3:
            return None, None, None, None, len(v), "premalo po čiščenju"

        avg_m2_upr_v   = v["CENA_M2_UPR"].dropna().mean()
        avg_m2_dela_v  = v["CENA_M2_DELA"].dropna().mean()

        razlika = max(skupna - upr, 0)

        if parcela > 0:
            # Cena/m2 glede na skupno + parcela
            v["CENA_M2_SKUPAJ"] = v["CENA"] / (v["POVRSINA_ZA_IZRACUN"] + v["PARCELA"].fillna(0))
            v_clean = ocisti(v.dropna(subset=["CENA_M2_SKUPAJ"]), "CENA_M2_SKUPAJ")
            avg_m2_skupaj = v_clean["CENA_M2_SKUPAJ"].mean()
            cena = avg_m2_skupaj * (skupna + parcela)
            cena_lo = v_clean["CENA_M2_SKUPAJ"].quantile(0.25) * (skupna + parcela)
            cena_hi = v_clean["CENA_M2_SKUPAJ"].quantile(0.75) * (skupna + parcela)
            opis = f"avg {avg_m2_skupaj:,.0f} €/m² × {skupna + parcela:.0f} m² (skupna+atrij)"
        else:
            cena_upr  = avg_m2_upr_v  * upr
            cena_razl = avg_m2_dela_v * razlika
            cena      = cena_upr + cena_razl
            cena_lo   = v["CENA_M2_UPR"].quantile(0.25) * upr + v["CENA_M2_DELA"].dropna().quantile(0.25) * razlika
            cena_hi   = v["CENA_M2_UPR"].quantile(0.75) * upr + v["CENA_M2_DELA"].dropna().quantile(0.75) * razlika
            opis = (f"{avg_m2_upr_v:,.0f} €/m² × {upr} m² (upr.) + "
                    f"{avg_m2_dela_v:,.0f} €/m² × {razlika:.0f} m² (razlika do skupne)")

        return cena, cena_lo, cena_hi, opis, len(v), None

    # Poišči vzorec po lokacijskih fallbackih
    koncni = None
    for lok_ime2, lok_vzorec2 in lokacije2:
        vzorec2, pov_info2 = filtriraj2(lok_vzorec2, rc_upr, rc_leto_izgr2)
        cena2, lo2, hi2, opis2, n2, napaka2 = izracunaj(vzorec2, rc_upr, rc_skupna, rc_parcela2)
        if cena2 is not None:
            koncni = (cena2, lo2, hi2, opis2, n2, lok_ime2, pov_info2)
            break

    if koncni is None:
        st.warning("Premalo primerljivih poslov za oceno. Poskusi z manj specifičnimi filtri.")
    else:
        cena2, lo2, hi2, opis2, n2, lok2, pov2 = koncni
        st.success(f"### Ocenjena vrednost: {cena2:,.0f} €")
        k1, k2, k3 = st.columns(3)
        k1.metric("Spodnji razpon", f"{lo2:,.0f} €")
        k2.metric("Zgornji razpon", f"{hi2:,.0f} €")
        k3.metric("Poslov v vzorcu", str(n2))
        st.caption(
            f"Lokacija: {lok2} · {pov2} · leto ±10/15 let · brez top/bottom 10% · {opis2}"
        )
