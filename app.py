...
def load_data():
    ...
    # Cena/m2 z atrijem: skupna površina dela stavbe + parcela (če > 0)
    df["POVRSINA_Z_ATRIJEM"] = df["POVRSINA_DELA_STAVBE"].fillna(0) + df["PARCELA"].fillna(0)
    df["CENA_M2_ATRIJ"] = df.apply(
        lambda r: r["CENA"] / r["POVRSINA_Z_ATRIJEM"]
        if pd.notna(r["CENA"]) and r["POVRSINA_Z_ATRIJEM"] > 0 else np.nan,
        axis=1
    )
    ...
    # LETO: zaokroži na int da ni float problemov
    if "LETO" in df.columns:
        df["LETO"] = df["LETO"].where(df["LETO"].between(2000, 2030)).round().astype("Int64")
...
with st.sidebar:
    ...
    if "LETO" in df.columns:
        # Pridobi vsa leta ki so v podatkih, ne glede na NaN
        leta_raw = df["LETO"].dropna().unique()
        leta_posla = sorted([int(l) for l in leta_raw])
...
avg_m2_atrij = filt["CENA_M2_ATRIJ"].dropna().mean()
med_m2_atrij = filt["CENA_M2_ATRIJ"].dropna().median()
...
with c3:
    st.markdown(f"""<div class='metric-card'>
        <div class='metric-label'>Povp. cena / m² (z atrijem)</div>
        <div class='metric-value'>{avg_m2_atrij:,.0f} €</div>
        <div class='metric-sub'>mediana {med_m2_atrij:,.0f} €/m²</div>
    </div>""", unsafe_allow_html=True)
with c4:
    st.markdown(f"""<div class='metric-card'>
...
tabela_cols += ["POVRSINA_DELA_STAVBE", "POVRSINA_ZA_IZRACUN", "PARCELA",
                "CENA", "CENA_M2_UPR", "CENA_M2_DELA", "CENA_M2_ATRIJ",
                "DEJANSKA_RABA_DELA_STAVBE", "LEGA_DELA_STAVBE_V_STAVBI"]
col_names   += ["Površina", "Uporabna površina", "Dodatna parcela (atrij ipd.)",
                "Cena", "Cena/m² uporabne", "Cena/m² skupne", "Cena/m² z atrijem/parkingom",
                "Raba", "Lega"]
...
tabela["CENA_M2_ATRIJ"] = tabela["CENA_M2_ATRIJ"].apply(
    lambda x: f"{x:,.0f} €/m²" if pd.notna(x) else "")
