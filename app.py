...
if r2c5.button("🔍 Oceni ceno", key="rc_btn"):
    ...
    # Filter parcele: ±50% če je vnesena, sicer ignoriraj
    def filtriraj_parcelo(vzorec, parcela):
        if parcela <= 0 or "PARCELA" not in vzorec.columns:
            return vzorec
        mask = vzorec["PARCELA"].between(parcela * 0.5, parcela * 1.5)
        sub = vzorec[mask]
    ...
    for lok_ime, lok_vzorec in lokacije:
        ...
        vzorec = filtriraj_parcelo(vzorec, rc_parcela)
    ...
    if rezultat:
        ...
        avg_m2_r = vzorec_r["CENA_M2_UPR"].quantile([0.15, 0.85])
        cena_lo  = avg_m2_r.iloc[0] * rc_povrsina
        cena_hi  = avg_m2_r.iloc[1] * rc_povrsina
        avg_m2_val = vzorec_r[vzorec_r["CENA_M2_UPR"].between(
            vzorec_r["CENA_M2_UPR"].quantile(0.15),
            vzorec_r["CENA_M2_UPR"].quantile(0.85)
        )]["CENA_M2_UPR"].mean()
        ...
        st.markdown(f"""<div class='realna-cena-card'>
            <div style="font-size:12px; color:#666; margin-bottom:6px;">
                ...
                · lokacija: <b>{lok_r}</b>
                · površina ±25% · leto izgradnje ±15 let
