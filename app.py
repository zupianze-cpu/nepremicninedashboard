...
if r2c5.button("🔍 Oceni ceno", key="rc_btn"):
    ...
    def filtriraj_povrsino(vzorec, pov, razpon=0.10):
        mask = vzorec["POVRSINA_ZA_IZRACUN"].between(pov * (1-razpon), pov * (1+razpon))
        sub = vzorec[mask]
        if len(sub) >= MIN_VZOREC:
            return sub
        # Premalo vzorcev — razširi na ±25%
        mask25 = vzorec["POVRSINA_ZA_IZRACUN"].between(pov * 0.75, pov * 1.25)
        sub25 = vzorec[mask25]
        return sub25 if len(sub25) >= MIN_VZOREC else vzorec

    # ── Osnovna ocena (brez parcele) ──────────────────────────────────────────
    rezultat = None
    for lok_ime, lok_vzorec in lokacije:
        vzorec = filtriraj_leto(lok_vzorec, rc_leto_izgr)
        vzorec = filtriraj_povrsino(vzorec, rc_povrsina)
    ...
    if not rezultat:
        ...
        if rc_parcela > 0 and "PARCELA" in baza.columns:
            # Vzorec z primerljivo parcelo (±50%) + isti lokacijski filter
            lok_vzorec_parc = rezultat[3]  # isti vzorec kot osnova
            mask_parc = lok_vzorec_parc["PARCELA"].between(rc_parcela * 0.5, rc_parcela * 1.5)
            vzorec_parc = lok_vzorec_parc[mask_parc]

            if len(vzorec_parc) >= 5:
                # Dovolj vzorcev — računaj direktno iz teh
                p15p = vzorec_parc["CENA_M2_UPR"].quantile(0.15)
                p85p = vzorec_parc["CENA_M2_UPR"].quantile(0.85)
                ociscen_parc = vzorec_parc[vzorec_parc["CENA_M2_UPR"].between(p15p, p85p)]
                avg_m2_parc  = ociscen_parc["CENA_M2_UPR"].mean()
                cena_r       = avg_m2_parc * rc_povrsina
                avg_m2_val   = avg_m2_parc
                cena_lo      = p15p * rc_povrsina
                cena_hi      = p85p * rc_povrsina
                parcela_info = f"· {len(vzorec_parc)} poslov z atrij/parking ±50%"

            elif len(vzorec_parc) >= 1:
                # 1–4 vzorcev — računaj po skupni površini (upr + parcela)
                skupna_pov = rc_povrsina + rc_parcela
                p15p = vzorec_parc["CENA_M2_UPR"].quantile(0.15)
                p85p = vzorec_parc["CENA_M2_UPR"].quantile(0.85)
                ociscen_parc = vzorec_parc[vzorec_parc["CENA_M2_UPR"].between(p15p, p85p)]
                if len(ociscen_parc) == 0:
                    ociscen_parc = vzorec_parc
                avg_m2_parc  = ociscen_parc["CENA_M2_UPR"].mean()
                cena_r       = avg_m2_parc * skupna_pov
                avg_m2_val   = avg_m2_parc
                cena_lo      = p15p * skupna_pov if p15p > 0 else cena_r * 0.85
                cena_hi      = p85p * skupna_pov if p85p > 0 else cena_r * 1.15
                parcela_info = f"· {len(vzorec_parc)} vzorec/vzorca z atrij/parking (skupna površina {skupna_pov:.0f} m²)"

            else:
                # Nič vzorcev — parcela vrednoti po polovici avg_m2 uporabne površine
                vrednost_parcele = avg_m2_val * rc_parcela * 0.5
                dodatek_parcela  = vrednost_parcele
        ...
        # ── Prikaz ────────────────────────────────────────────────────────────
        parc_vrstica = f"<br><span style='font-size:11px; color:#888;'>Atrij/parking: {parcela_info}</span>" if parcela_info else ""

        st.markdown(f"""<div class='realna-cena-card'>
            <div style="font-size:12px; color:#666; margin-bottom:6px;">
                Ocena na podlagi <b>{n_r} primerljivih poslov</b> v letu 2025
                · lokacija: <b>{lok_r}</b>
                · površina ±10% (±25% fallback) · leto ±15 let
                · brez top/bottom 15% cen
                {parc_vrstica}
            </div>
            <div class='glavna'>{cena_r:,.0f} €</div>
            <div class='sub'>
                Razpon: {cena_lo:,.0f} € – {cena_hi:,.0f} € &nbsp;·&nbsp;
                Povp. {avg_m2_val:,.0f} €/m² &nbsp;·&nbsp;
                Za {rc_povrsina} m²
            </div>
        </div>""", unsafe_allow_html=True)
